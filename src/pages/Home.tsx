...
import { ChevronRight, ShieldCheck, Heart, TrendingUp, ArrowRight } from 'lucide-react';
    ...
    <section className="relative min-h-screen flex items-center pt-20 overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-3xl">
          ...
          >
            <span className="inline-block px-4 py-1.5 bg-brand-point/10 text-brand-dark text-xs font-bold tracking-widest uppercase rounded-full mb-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to="/business" className="px-8 py-4 bg-brand-point text-white font-bold rounded-xl hover:bg-brand-dark transition-all flex items-center justify-center gap-2 shadow-lg shadow-brand-point/20">
                사업 분야 보기 <ArrowRight size={20} />
              </Link>
              <Link to="/about" className="px-8 py-4 bg-white text-gray-900 font-bold rounded-xl border border-gray-200 hover:border-brand-point transition-all flex items-center justify-center">
                회사 소개
              </Link>
            </div>
          </motion.div>
...
const FeatureCard = ({ icon: Icon, title, description }: any) => (
  <div className="p-8 bg-white rounded-2xl border border-gray-100 hover:border-brand-point/30 hover:shadow-xl hover:shadow-brand-point/5 transition-all group">
    ...
    <div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <FeatureCard 
              icon={ShieldCheck} 
              title="신뢰 (Trust)" 
              description="검증된 품질의 제품만을 공급하여 의료진과 환자 모두가 안심할 수 있는 환경을 만듭니다."
            />
            <FeatureCard 
              icon={Heart} 
              title="정직 (Integrity)" 
              description="투명한 유통 과정과 정직한 비즈니스 파트너십을 통해 지속 가능한 성장을 추구합니다."
            />
            <FeatureCard 
              icon={TrendingUp} 
              title="성실 (Sincerity)" 
              description="고객의 요구에 신속하고 정확하게 대응하며, 최상의 서비스를 제공하기 위해 끊임없이 노력합니다."
            />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-4xl font-bold mb-6">주요 사업 분야</h2>
              <p className="text-gray-500">유성메디칼이 제공하는 전문적인 의료 솔루션입니다.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {BUSINESS_ITEMS.slice(0, 4).map((item, idx) => (
                <div key={idx} className="bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all group">
                  ...
                  <div className="p-6">
                    <h4 className="font-bold text-gray-900 mb-1">{item.brand}</h4>
                    <p className="text-xs text-gray-500 truncate">{item.items.slice(0, 3).join(', ')} 등</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="text-center mt-12">
              <Link to="/business" className="inline-flex items-center gap-2 text-brand-point font-bold hover:gap-4 transition-all">
                전체 사업 내용 보기 <ChevronRight size={20} />
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-brand-point">
        <div className="container mx-auto px-6 text-center">
          ...
          <Link to="/contact" className="px-10 py-5 bg-white text-brand-point font-bold rounded-2xl hover:bg-gray-50 transition-all shadow-xl">
            지금 문의하기
          </Link>
        </div>
      </section>
